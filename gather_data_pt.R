...
if(using_googlesheets){
  ...
  position_data <- read_csv("csvs_pt/positions.csv")
  skills        <- read_csv("csvs_pt/language_skills.csv")
  text_blocks   <- read_csv("csvs_pt/text_blocks.csv")
  contact_info  <- read_csv("csvs_pt/contact_info.csv", skip = 1)
  comp_education  <- read_csv("csvs_pt/complementary_education.csv")
  events  <- read_csv("csvs_pt/events.csv")
  publications  <- read_csv("csvs_pt/publications.csv")
